<template>
            ...
            <div class="nav-list">
                ...
                <template v-if="userData">
                    ...
                    >
                        ...
                        <template v-else>
                            <img
                                src="../assets/user_profile_assets/basic_profile_img.svg"
                            />
                        </template>
                        <span class="list_user_profile_text">
                            <p class="list_user_profile_name">
                                {{ userData.username }}
                            </p>
                            <p class="list_user_profile_department">
                                {{ department_map[userData.department] }}
                            </p>
                        </span>
                    </router-link>
                </template>
    ...
    <transition name="nav-bg">
        <div
            v-if="isSidebarShow"
            @click="$store.commit('sidebarOnOff')"
            class="sidebar-bg"
        ></div>
    </transition>
</template>
...
export default {
    ...
    methods: {
        hideSidebarOnMobile() {
            if (this.isMobileWindow) this.$store.commit("sidebarOff");
        },
    },
    computed: {
        ...mapState([
...
.router-link-exact-active img {
    filter: invert(57%) sepia(64%) saturate(4033%) hue-rotate(198deg)
